...
{
    public function __invoke(ListWorkshopsIndexRequest $request, WorkshopAdminFilters $workshopAdminFilters): Response
    {
        $user = $request->user();
        assert($user !== null);

        $data = $workshopAdminFilters->index($request->validated());
