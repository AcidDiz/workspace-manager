...
| POST   | `/app/workshops/{workshop}/registrations` | `app.workshops.registrations.attach` | Same group as app index: **`auth`**, **`verified`**, **`can:viewAny,Workshop`**, **`can:attachRegistration,workshop`**. Attach current user as **confirmed** when the workshop is upcoming and has capacity; if full, create a **waiting_list** row instead; otherwise **302** back with error toast for domain failures (for example overlap or closed workshop). |
